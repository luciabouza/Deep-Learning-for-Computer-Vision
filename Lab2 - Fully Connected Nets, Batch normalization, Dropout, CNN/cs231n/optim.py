...
def rmsprop(w, dw, config=None):
    ...
    gradSquared = config['decay_rate']*config['cache']  + (1 - config['decay_rate'])* dw * dw
    next_w = w - ( config["learning_rate"] * dw / (np.sqrt(gradSquared) + config['epsilon']) )
    config['cache'] = gradSquared
